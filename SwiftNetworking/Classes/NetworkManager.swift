...
func customSession() -> Session {
    let serverTrustManager = ServerTrustManager(allHostsMustBeEvaluated: false,
                                                        evaluators: ["xr-studio.sensetime.com": DisabledTrustEvaluator()])
...
//MARK: 设置ssl
let customSession2 : Session = {
   //证书数据
   func certificate() -> SecCertificate? {
       let filePath = Bundle.main.path(forResource: "nginx", ofType: "cer")
       if filePath == nil {
           return nil
       }
   ...
   let trustManager = ServerTrustManager(evaluators: ["xr-studio.sensetime.com" : trusPolicy])
